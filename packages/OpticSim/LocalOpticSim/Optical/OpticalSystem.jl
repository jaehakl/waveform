...
struct AxisymmetricOpticalSystem{T,C<:CSGOpticalSystem{T}} <: AbstractOpticalSystem{T}
    ...
    function AxisymmetricOpticalSystem{T}(
        ...
        for i in 2:nrow(prescription)-1
            ...
            elseif surface_type == "Aspheric"
                ...
                error(
                    ...
                    "please create an issue at https://github.com/microsoft/LocalOpticSim.jl/issues/new."
                )
            end
        ...
        system = CSGOpticalSystem(
            LocalOpticSim.LensAssembly(elements...), det, detectorpixelsx, detectorpixelsy, D; temperature, pressure
        )
...
function traceMT(
    ...
    det = LocalOpticSim.detectorimage(copies[1][1])
    # sum all the copies
    @inbounds for i in 2:N
        LocalOpticSim.sum!(det, LocalOpticSim.detectorimage(copies[i][1]))
    end
