...
"""
abstract type AbstractSource{T} <: LocalOpticSim.OpticalRayGenerator{T} end
...
function Emitters.generate(s::Source{T}, state::SourceGenerationState{T} = SourceGenerationState(-1, -1, zeros(Vec3{T}))) where {T<:Real}
    ...
    return (LocalOpticSim.OpticalRay(ray, power, wavelength; sourcenum=s.sourcenum), SourceGenerationState(state.n+1, origin_index, o))
end
