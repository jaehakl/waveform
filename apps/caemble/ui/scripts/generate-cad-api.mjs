...
function validateMaterialParameterCatalog(
  ...
  if (
    ...
    || materialParameterCatalog.catalog_version !== '0.1-draft'
  ...
  if (
    ...
    || !Array.isArray(materialParameterCatalog.model_namespace_examples)
  ) {
  ...
  if (
    ...
    || !Object.isFrozen(materialParameterCatalog.model_namespace_examples)
  ) {
    throw new Error('Material catalog metadata must be frozen.')
  }
  if (
    !Array.isArray(materialParameterData)
    || materialParameterData.length !== 260
    || materialParameterCatalog.properties !== materialParameterData
    || !Object.isFrozen(materialParameterData)
  ) {
    throw new Error('Material catalog must expose its frozen 260-property aggregate as properties.')
  }
...
function materialParameterCatalogMarkdown(
  ...
  lines.push(
    ...
    '',
    ...materialParameterCatalog.model_namespace_examples.map((example) => `- ${markdownCode(example)}`),
  )
  return `${lines.join('\n')}\n`
}
...
const [quantityModule, materialModule] = await Promise.all([
  loadBundledModule(path.join(root, 'src/quantitykind/data.ts')),
  loadBundledModule(path.join(root, 'src/material/data.ts')),
])
...
if (!materialParameterCatalog || !materialParameterData || !materialParameterDomains) {
...
if (quantityKindNames.length !== 1_231) {
  throw new Error(`QuantityKind data must contain exactly 1,231 entries; found ${quantityKindNames.length}.`)
}
...
await Promise.all([
  ...
  emit(
    'src/material/material_parameter_catalog_v0.1.md',
