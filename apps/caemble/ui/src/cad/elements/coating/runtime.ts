...
import type { CompoundElementDefinition } from '../../evaluation/types'
import { coatingManifest } from './definition'
...
export function createCoatingGeometries(geometry: unknown, offsets: unknown): CadGeom3[] {
  if (!Array.isArray(offsets) || offsets.length === 0) {
    throw new CadModelError('<coating> offsets must be a non-empty array.')
  }

  const signedOffsets = offsets as unknown[]
  signedOffsets.forEach((offset, index) => {
    if (typeof offset !== 'number' || !Number.isFinite(offset) || offset === 0) {
      throw new CadModelError(`<coating> offset at index ${index} must be a finite non-zero number.`)
    }
    if (index > 0 && offset <= (signedOffsets[index - 1] as number)) {
      throw new CadModelError('<coating> offsets must be in strictly increasing order.')
    }
  })
  const validOffsets = signedOffsets as number[]

  if (!geometries.geom3.isA(geometry)) {
    throw new CadModelError('<coating> child Geometry must be a valid closed geom3 solid.')
  }
  ...
  const triangles: Triangle[] = polygons.map((polygon) => {
    if (polygon.vertices.length !== 3) {
      throw new CadModelError('<coating> child Geometry could not be triangulated.')
    }
  ...
  triangles.forEach(({ indices, normal }) => {
    indices.forEach((pointIndex, cornerIndex) => {
      ...
      if (!Number.isFinite(weight) || weight <= 0) {
        throw new CadModelError('<coating> child Geometry contains a degenerate triangle.')
      }
  ...
  const unitDisplacements = points.map((_, pointIndex) => {
    ...
    if (!Number.isFinite(averageNormalLength) || averageNormalLength === 0 || totalWeight === 0) {
      throw new CadModelError(`<coating> could not calculate a stable offset at vertex ${pointIndex}.`)
    }
    ...
    triangles.forEach(({ indices, normal }, triangleIndex) => {
      ...
      if (!Number.isFinite(signedArea) || signedArea <= minimumArea) {
        throw new CadModelError(
          `<coating> offset ${offset} creates a degenerate or inverted surface at triangle ${triangleIndex}.`,
        )
      }
    })
  ...
  return boundaries.slice(0, -1).map((innerOffset, layerIndex) => {
    ...
    if (!innerPoints || !outerPoints) {
      throw new CadModelError(`<coating> could not create layer [${innerOffset}, ${outerOffset}].`)
    }

    const outerStart = innerPoints.length
    const coating = primitives.polyhedron({
    ...
    try {
      geometries.geom3.validate(coating)
      const volume = measurements.measureVolume(coating)
      if (!Number.isFinite(volume) || volume <= 0) throw new Error('invalid coating volume')
    } catch {
      throw new CadModelError(`<coating> generated an invalid layer [${innerOffset}, ${outerOffset}].`)
    }
    return coating
  })
}

export const coatingDefinition = {
  kind: 'compound',
  tag: coatingManifest.tag,
  manifest: coatingManifest,
  evaluate(node, context) {
    if (node.children.length !== 1) {
      throw new CadModelError('<coating> requires exactly one direct child Geometry.')
    }
    ...
    if (context.inheritedMaterials?.length !== node.props.offsets.length) {
      throw new CadModelError('<coating> requires exactly one inherited Material per offset.')
    }

    const parts = context.evaluate(node.children[0], context.inheritedMaterials)
    if (parts.length !== 1) {
      throw new CadModelError('<coating> child Geometry must evaluate to exactly one solid.')
    }

    return createCoatingGeometries(parts[0].geometry, node.props.offsets).map((geometry, index) => ({
      geometry,
      material: context.inheritedMaterials![index],
    }))
  },
} satisfies CompoundElementDefinition<'coating'>
