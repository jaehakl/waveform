...
describe('runtime source-map diagnostics', () => {
  it('maps a new Function stack frame back to the TSX source range', () => {
    const emitted = ts.transpileModule(`const fail = () => {
...
`, {
    ...
    const project: CompiledCadProjectV2 = {
      apiVersion: 2,
    ...
    expect(runtimeDiagnostic(error, project)).toMatchObject({
      code: 'CAD_RUNTIME',
      file: 'structure.tsx',
      message: 'boom',
      phase: 'runtime',
  ...
  it('ignores stack frames from another compiled project', () => {
    const project: CompiledCadProjectV2 = {
      ...
      compilerVersion: CAD_COMPILER_VERSION,
      entryFile: 'structure.tsx',
      modules: { 'structure.tsx': { code: '' } },
      sourceHash,
    }
    const error = new Error('boom')
    error.stack = `Error: boom\n    at caemble://${'d'.repeat(64)}/structure.tsx:3:1`

    expect(runtimeDiagnostic(error, project)).toBeUndefined()
  })
})
