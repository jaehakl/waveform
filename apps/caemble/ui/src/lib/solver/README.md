...
Every solver is a folder under `src/lib/solver/modules` and must expose one `SolverModule` from its `index.ts`.
...
Register the exported module once in `src/lib/solver/modules/index.ts`. The controller, Worker preflight, and Solver Spec UI all consume that same registration; do not add solver-specific branches to UI or Worker code.
