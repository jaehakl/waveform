...
const catalogs = [
  { count: catalogCounts.cad, description: 'primitive와 operation 문법', href: '/catalog/cad', icon: Boxes, label: 'CAD 요소' },
  { count: catalogCounts.materials, description: 'parameter 258개 · relation 2개', href: '/catalog/materials', icon: Layers3, label: 'Material' },
  { count: catalogCounts.quantityKinds, description: 'domain·tensor order·UCUM unit', href: '/catalog/quantity-kinds', icon: Gauge, label: 'Quantity Kind' },
  { count: catalogCounts.solvers, description: 'parameter·method·result 계약', href: '/catalog/solvers', icon: FlaskConical, label: 'Solver' },
] as const

function RecentWork() {
  const { isAuthenticated, user } = useAuth()
  const request = getListRequest('mine')
  const structureQuery = useQuery({ queryKey: ['work', 'structures'], queryFn: () => dbTables.Structure.listRows(request), enabled: isAuthenticated })
  const sampleQuery = useQuery({ queryKey: ['work', 'samples'], queryFn: () => dbTables.Sample.listRows(request), enabled: isAuthenticated })
  const experimentQuery = useQuery({ queryKey: ['work', 'experiments'], queryFn: () => dbTables.Experiment.listRows(request), enabled: isAuthenticated })
  const setupQuery = useQuery({ queryKey: ['work', 'setups'], queryFn: () => dbTables.Setup.listRows(request), enabled: isAuthenticated })
  ...
  return (
    <section aria-labelledby="recent-work-title">
      <div className="mb-4 flex items-center justify-between"><div><h3 className="font-semibold" id="recent-work-title">내 최근 작업</h3><p className="text-sm text-muted-foreground">{user?.display_name || user?.email} 계정의 리소스입니다.</p></div><Button asChild size="sm" variant="ghost"><Link to="/account">전체 보기<ArrowRight /></Link></Button></div>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {resources.map(({ label, query }) => (
          <Card key={label}>
            <CardHeader className="pb-3"><CardDescription>{label}</CardDescription>{query.isLoading ? <Skeleton className="h-7 w-14" /> : <CardTitle className="text-2xl">{query.data?.total ?? 0}</CardTitle>}</CardHeader>
            <CardContent className="text-xs text-muted-foreground">{query.isError ? '목록을 불러오지 못했습니다.' : query.data?.items[0] ? `최근 업데이트 · ${new Date(query.data.items[0].updated_at ?? Date.now()).toLocaleDateString('ko-KR')}` : '아직 저장된 항목이 없습니다.'}</CardContent>
          </Card>
        ))}
      </div>
    </section>
