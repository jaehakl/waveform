...
from .auth import AuthState, login, logout, check_session
...
class LoginDialog(QDialog):
    def __init__(self, parent):
        ...
        if is_logged_in:
            self.email = QLabel()
            self.email.setText("로그인 계정 : "+user_info["name"])
            ...
            self.setLayout(layout)
            LineEditComp(self,
                onChange=AuthState().auth_host.set,
                props={"label":Prop("Auth Host"),"text":AuthState().auth_host})
