...
def set(d, omega_list):
    ...
    #RC parameters ("FDTD for Nth-order dispersive media" (1992), DOI : 10.1109/8.202707)
    dt = d.v["dt"]
    wp = d.v["omega_p"]
    vc = d.v["omega_c"]
    d.v["chi0"] = (wp**2)*dt/vc - ((wp/vc)**2)*(1-np.exp(-vc*dt))
    d.v["dchi0"] = -((wp/vc)*(1-np.exp(-vc*dt)))**2
    d.v["1/(e+ch)"] = 1.0/(d.v["eps_inf"] + d.v["chi0"])
...
def cal_E(d):
    if d.v["pml"] == "cpml":
        ...
        d.v["Ex"][1:-1,1:-1,1:-1] = ((1-d.CEX("metal"))*(d.V("Ex")+(dHa/d.V("KAPPA_E1")-dHb/d.V("KAPPA_E2")+d.V("PSI_E01")-d.V("PSI_E02"))*d.v["dt/eps0"]/d.CEX("eps"))
            +d.CEX("metal")*d.v["1/(e+ch)"]*(d.V("Ex")+(dHa/d.V("KAPPA_E1")-dHb/d.V("KAPPA_E2")+d.V("PSI_E01")-d.V("PSI_E02"))*d.v["dt/eps0"] - d.V("Jx")))
        d.v["Jx"][1:-1,1:-1,1:-1]=d.v["e^(-omega_c*dt)"]*d.V("Jx")-d.v["dchi0"]*d.V("Ex")
        ...
        d.v["Ey"][1:-1,1:-1,1:-1] = ((1-d.CEY("metal"))*(d.V("Ey")+(dHa/d.V("KAPPA_E2")-dHb/d.V("KAPPA_E0")+d.V("PSI_E12")-d.V("PSI_E10"))*d.v["dt/eps0"]/d.CEY("eps"))
            +d.CEY("metal")*d.v["1/(e+ch)"]*(d.V("Ey")+(dHa/d.V("KAPPA_E2")-dHb/d.V("KAPPA_E0")+d.V("PSI_E12")-d.V("PSI_E10"))*d.v["dt/eps0"] - d.V("Jy")))
        d.v["Jy"][1:-1,1:-1,1:-1]=d.v["e^(-omega_c*dt)"]*d.V("Jy")-d.v["dchi0"]*d.V("Ey")
        ...
        d.v["Ez"][1:-1,1:-1,1:-1] = ((1-d.CEZ("metal"))*(d.V("Ez")+(dHa/d.V("KAPPA_E0")-dHb/d.V("KAPPA_E1")+d.V("PSI_E20")-d.V("PSI_E21"))*d.v["dt/eps0"]/d.CEZ("eps"))
            +d.CEZ("metal")*d.v["1/(e+ch)"]*(d.V("Ez")+(dHa/d.V("KAPPA_E0")-dHb/d.V("KAPPA_E1")+d.V("PSI_E20")-d.V("PSI_E21"))*d.v["dt/eps0"] - d.V("Jz")))
        d.v["Jz"][1:-1,1:-1,1:-1]=d.v["e^(-omega_c*dt)"]*d.V("Jz")-d.v["dchi0"]*d.V("Ez")

    else:
        cH = d.dVdy("Hz",0)-d.dVdz("Hy",0)
        d.v["Ex"][1:-1,1:-1,1:-1]=((1-d.CEX("metal"))*(d.V("Ex")+cH*d.v["dt/eps0"]/d.CEX("eps"))
            + d.CEX("metal")*d.v["1/(e+ch)"]*(d.V("Ex")-d.V("Jx") + d.v["dt/eps0"]*cH))
        d.v["Jx"][1:-1,1:-1,1:-1]=d.v["e^(-omega_c*dt)"]*d.V("Jx")-d.v["dchi0"]*d.V("Ex")

        cH =d.dVdz("Hx",0)-d.dVdx("Hz",0)
        d.v["Ey"][1:-1,1:-1,1:-1]=((1-d.CEY("metal"))*(d.V("Ey")+cH*d.v["dt/eps0"]/d.CEY("eps"))
            + d.CEY("metal")*d.v["1/(e+ch)"]*(d.V("Ey")-d.V("Jy") + d.v["dt/eps0"]*cH))
        d.v["Jy"][1:-1,1:-1,1:-1]=d.v["e^(-omega_c*dt)"]*d.V("Jy")-d.v["dchi0"]*d.V("Ey")

        cH =d.dVdx("Hy",0)-d.dVdy("Hx",0)
        d.v["Ez"][1:-1,1:-1,1:-1]=((1-d.CEZ("metal"))*(d.V("Ez")+cH*d.v["dt/eps0"]/d.CEZ("eps"))
            + d.CEZ("metal")*d.v["1/(e+ch)"]*(d.V("Ez")-d.V("Jz") + d.v["dt/eps0"]*cH))
        d.v["Jz"][1:-1,1:-1,1:-1]=d.v["e^(-omega_c*dt)"]*d.V("Jz")-d.v["dchi0"]*d.V("Ez")
