...
import { GeometryPainter } from './qutat3d/3d/geometry_painter';
...
export class Experiment3D extends React.Component {
  ...
  render(){
    ...
    setupData.structureEvaluated.forEach((structure, index) => {
        ...
        geometry.lineColor = 0x000000
        geometry.areaColor = 0x000000
        geometry.opacity = 0.5
        geometry.paintingLine = true
        geometry.paintingArea = true
        geometries.push(geometry)      
    ...
    return (
      <Grid>
        <Checkbox checked={this.state.showSource} onChange={()=>{this.setState({showSource:!this.state.showSource})}}/> Source
        <Checkbox checked={this.state.showDetector} onChange={()=>{this.setState({showDetector:!this.state.showDetector})}}/> Detector
