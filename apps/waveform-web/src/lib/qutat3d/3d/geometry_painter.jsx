...
export class GeometryPainter extends React.Component {
  ...
  render(){
    if (typeof document !== "undefined") {
      ...
      geometries.map((unit) => {
        ...
        } else if (unit.component === 'so_revol_func'){
          return new SolRevolFunc(scene, unit).paint()
        } else {
          return new Sphere(scene, unit).paint()
        }
      });          
...
class SolRevolFunc extends Geometry {
  paint(){
    const pos = this.pos
    const [ h ] = this.size
    const rot_func_str = this.props[0][0]
    ...
    var vertices = []
    for (let z = 0; z < h+resolution; z+=resolution) {

      var pz = z - 0.5*h
      const abs = Math.abs

      var pr = Math.abs(eval(rot_func_str))
      let i_vertices = []
      for (let j = 0; j < n_phi; j++) {
        var phi = 2*Math.PI*j/n_phi
        var px = pr*Math.cos(phi)
        var py = pr*Math.sin(phi)
        i_vertices.push([px,py,pz])
      }
      vertices.push(i_vertices)
    }
    ...
    for (let i = 0; i < vertices.length-1; i++) {
      for (let j = 0; j < n_phi; j++) {
        ...
        points.push(new THREE.Vector3(...vertices[i][(j+1)%n_phi]))
        points.push(new THREE.Vector3(...vertices[i+1][j]))
        points.push(new THREE.Vector3(...vertices[i][(j+1)%n_phi]))
        points.push(new THREE.Vector3(...vertices[i+1][(j+1)%n_phi]))
      }
    }
    for (let j = 0; j < n_phi; j++) {  
      points.push(new THREE.Vector3(0,0,-0.5*h))
      points.push(new THREE.Vector3(...vertices[0][(j+1)%n_phi]))
      points.push(new THREE.Vector3(...vertices[0][j]))
    }
    for (let j = 0; j < n_phi; j++) {  
      points.push(new THREE.Vector3(0,0,0.5*h))
      points.push(new THREE.Vector3(...vertices[vertices.length-1][j]))
      points.push(new THREE.Vector3(...vertices[vertices.length-1][(j+1)%n_phi]))
    }

    var geometry = new THREE.BufferGeometry().setFromPoints( points );
    var area_mesh = new THREE.Mesh( geometry, new THREE.MeshBasicMaterial( { color: this.areaColor } ) );
    this.rotateMesh(area_mesh, this.rotations)
    area_mesh.position.set(pos[0],pos[1],pos[2])
